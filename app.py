...
def add_chords_to_lyrics(lyrics_text: str, title: str, density: int = 5) -> str:
    ...
    templates = {
        "ESTUVE": {"chords": STAGE_CHORD_TEMPLATES.get("ESTUVE", []), "density": 5},
        "ESO Y MÁS": {"chords": STAGE_CHORD_TEMPLATES.get("ESO Y MÁS", []), "density": 4},
        "TATUAJES": {"chords": STAGE_CHORD_TEMPLATES.get("TATUAJES", []), "density": 5},
    }
...
with col2:
    if "processed" in st.session_state:
        ...
        st.markdown(f"<h2 style='margin-bottom: 8px;'>{title.upper()}</h2>", unsafe_allow_html=True)
        if artist and artist != "Artist":
            st.markdown(f"<h4 style='margin-top: 0; color: #666;'>{artist}</h4>", unsafe_allow_html=True)

        # === REGENERATE BUTTON - Better placement ===
        if st.button("🔄 Re-apply Chords with New Density", type="secondary", use_container_width=True):
            if "original_lyrics" in st.session_state:
                with st.spinner("Regenerating chords..."):
        ...
        # === DISPLAY LYRICS (Fixed chord display) ===
        ...
        with col_b:
            if st.button("🖨️ Print / Save as PDF", use_container_width=True):
                printable_title = title.upper()
                show_ts = show_timestamps
                
                printable_html = f"""
                <!DOCTYPE html>
                <html>
                <head>
                    <title>{printable_title} - {artist}</title>
                    <style>
                        @media print {{
                            body {{ 
                ...
                <body>
                    <h1>{printable_title}</h1>
                    <h2 style="text-align:center; color:#555;">{artist}</h2>
                    <hr>
                """

                for line in lyrics.split("\n"):
                    if not line.strip():
                        printable_html += "<div class='line'><br></div>"
                        continue
                    ...
                    ts_html = f'<span class="timestamp">{timestamp}</span>' if show_ts and timestamp else ""
                    printable_html += f'<div class="line">{ts_html}{content_html}</div>'
                ...
                js_code = f"""
                    <script>
                        (function() {{
                            const htmlContent = {repr(printable_html)};
